...
class Asset(models.Model):
    ...
    # ✅ All quantity logic is handled in views.py — no auto update in model
    ...
    connectivity_type = models.CharField(
        max_length=50, choices=CONNECTIVITY_CHOICES, null=True, blank=True
    )
    ...
    warranty_status = models.CharField(
        max_length=20, choices=WARRANTY_STATUS_CHOICES, null=True, blank=True
    )
    ...
    # ── ADDITIONAL ────────────────────────────────────────────────────────────
    attachment         = models.FileField(upload_to="asset_attachments/",  null=True, blank=True)
    warranty_documents = models.FileField(upload_to="asset_warranty_docs/", null=True, blank=True)
    remarks            = models.TextField(null=True, blank=True)
    created_at         = models.DateTimeField(auto_now_add=True)
    updated_at         = models.DateTimeField(auto_now=True)

    # ✅ NO save() override — all status/quantity logic is in views.py
...
class AssetDetails(models.Model):
    ...
    issued_by = models.ForeignKey(
        ...
        null=True, related_name='assets_issued'
    )

    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)

    # ✅ NO save() override — all quantity logic is in views.py
...
class PurchaseRequest(models.Model):
    ...
    triggered_by = models.CharField(max_length=10, choices=TRIGGERED_BY_CHOICES)
